...
# **Params:** fs=360 Hz (MIT-BIH standard), order=4 (good roll-off without ringing).

# %%
FS = 360       # Sampling freq (Hz) — MIT-BIH standard
...
# 3. Verify with **full-vector Euclidean distance** (z-score normalised)
...
# %%
# Full-vector Euclidean distance (z-score normalised to equalise feature scales)
mean_vals = feature_map.mean(axis=0)
std_vals  = feature_map.std(axis=0) + 1e-9
ali_n = (ali_features - mean_vals) / std_vals
mohamed_n = (mohamed_features - mean_vals) / std_vals
test_n = (test_features - mean_vals) / std_vals

eucl_ali = np.sqrt(np.sum((test_n - ali_n)**2))
eucl_mohamed = np.sqrt(np.sum((test_n - mohamed_n)**2))
